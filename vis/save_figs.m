function save_figs( figDir, h, matfigDir, epsfigDir )
%SAVE_FIGS  save figures as .png, .fig, and .eps
%
%   SAVE_FIGS( figDir ) save all open figures to figDir as .png, 
%   and, optionally, to .fig and .eps
%
%   SAVE_FIGS( ..., h ) saves all figures in figure handles, h
%
%   SAVE_FIGS( ..., h, matfigDir ) saves .fig files to matfigDir
%
%   SAVE_FIGS( ..., h, matfigDir, epsfigDir ) saves .eps files to epsfigDir
...
if ~exist( 'matfigDir', 'var' )
    matfigDir = [];
end

if ~exist( 'epsfigDir', 'var' )
    epsfigDir = [];
end
...
if ~isempty( matfigDir )
    if ~exist( matfigDir, 'dir' )
        mkdir( matfigDir );
    end
end

if ~isempty( epsfigDir )
    if ~exist( epsfigDir, 'dir' )
        mkdir( epsfigDir );
    end
end
...
for i=1:length(h)
    ...
    if ~isempty( epsfigDir )
        save_eps( h(i), fullfile( epsfigDir, [ figName, '.eps' ] ) );
    end
end
...
function save_eps( hFig, fileName )
% SAVE_EPS  save document-ready figure as .eps
